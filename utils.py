...
def createVocabularyChinese(input_path, output_path, no_pad=False):
...
def bioRestore(slot_outputs, input_words):
    slots = {}
    if len(slot_outputs) !=len(input_words):
        print('slots_outputs not equal to input_word in length')
        return slots
    else:
        position = []
        pattern = re.compile('^B')
        for index, item in enumerate(slot_outputs):
            if pattern.search(item) != None:
                position.append(index)

        for index, pos in enumerate(position):
            item = slot_outputs[pos]
            key = item[2:]
            value = []
            end = index
            if index < len(position)-1:
                ...
                while True:
                    if end < position[index+1]:
                        if slot_outputs[end][:1] == 'I':
                            end +=1
                        else:
                            break
                    else:
                        break
                value = input_words[position[index]:end]
                value = ''.join(value)
                slots.update({key:value})
            else:
                end = position[index] + 1
                while True:
                    if end < len(slot_outputs):
                        if slot_outputs[end][:1] == 'I':
                            end +=1
                        else:
                            break
...
class DataProcessor(object):
    def __init__(self, in_path, domain_path, slot_path, intent_path, in_vocab, domain_vocab, slot_vocab, intent_vocab):
        self.__fd_in = open(in_path, 'r')
        self.__fd_domain = open(domain_path, 'r')
        self.__fd_slot = open(slot_path, 'r')
        self.__fd_intent = open(intent_path, 'r')
        self.__in_vocab = in_vocab
        self.__domain_vocab = domain_vocab
    ...
    def get_batch(self, batch_size):
        ...
        for i in range(batch_size):
            ...
            domain = self.__fd_domain.readline()
            ...
            inp = sentenceToIdsChinese(inp, self.__in_vocab)
            domain = sentenceToIds(domain, self.__domain_vocab)
        ...
        return in_data, slot_data, slot_weight, length, domain_data, intents, in_seq, domain_seq, slot_seq, intent_seq


class TestDataProcesser(object):

    def __init__(self, in_path, in_vocab, domain_vocab, slot_vocab, intent_vocab):
        self.__fd_in = open(in_path, 'r')
        self.__in_vocab = in_vocab
        self.__domain_vocab = domain_vocab
        self.__slot_vocab = slot_vocab
